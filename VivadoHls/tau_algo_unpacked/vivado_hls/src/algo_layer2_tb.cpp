...
int main(int argc, char ** argv) {
  ...
  algo_test_layer2(ch_link_in, ne_link_in, em_link_in, mu_link_in,  link_out);//pfch_sel,pfpho_sel,pfne_sel,pfmu_sel);
  for(int idepth = 0; idepth < 1; idepth++) {
    axi_t tmp_out; 
    link_out.read(tmp_out);
    std::cout << " out " << tmp_out.data << std::endl;
    for(int i0 = 0; i0 < NPART; i0++) chparts[idepth][i0] = tmp_out.data.range(63*(i0+1),64*(i0));
    for(int i0 = 0; i0 < NPART; i0++) { 
     tmpaxi_t pTmp = chparts[idepth][i0];
